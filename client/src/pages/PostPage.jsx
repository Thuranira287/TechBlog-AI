...
const PostPage = () => {
  ...
  // Define constants AFTER post is loaded
  ...
  const sharePost = (platform) => {
    ...
    const shareUrls = {
      ...
      )}${post.tags && post.tags.length > 0 ? `&hashtags=${encodeURIComponent(post.tags.slice(0, 3).join(','))}` : ''}`,
              ...
              <div className="relative" ref={shareMenuRef}>
                ...
                {showShareMenu && (
                  <>
                    ...
                    {/* Share Menu */}
                    <div className="fixed bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-2xl py-4 z-50 md:absolute md:left-auto md:right-0 md:bottom-auto md:mt-2 md:w-48 md:rounded-lg md:shadow-lg md:border md:border-gray-200 animate-slideUpMobile md:animate-fadeIn">
                      {/* Handle bar for mobile */}
                      <div className="md:hidden w-12 h-1 bg-gray-300 rounded-full mx-auto mb-4" />
                      
                      {/* Title for mobile */}
                      <h3 className="md:hidden text-lg font-semibold text-gray-900 px-6 mb-3">Share this post</h3>
                      
                      <button
                        onClick={() => sharePost('twitter')}
                        className="w-full px-6 md:px-4 py-3 md:py-2 text-left text-sm text-gray-700 hover:bg-gray-50 flex items-center space-x-3"
                      >
