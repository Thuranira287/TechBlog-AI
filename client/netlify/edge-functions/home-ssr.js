...
export default async (request, context) => {
  try {
    ...
    if (url.pathname !== '/') return context.rewrite('/index.html');
    ...
    if (isAICrawler) {
      ...
      try {
        ...
        clearTimeout(timeoutId);
        console.error('[Edge-Home] Fetch error:', err.message);
    ...
    ${posts.slice(0, 10).map(post => `
      {
        ...
        "author": { "@type": "Person", "name": "${escapeJson(post.author_name || 'Admin')}" },
        "datePublished": "${post.published_at}",
        "description": "${escapeJson(post.excerpt || post.content || '')}",
    ...
    ${posts.length ? `<ul>
      ${posts.slice(0, 10).map(post => `
        <li>
          ...
          <br>${escapeHtml(post.content || post.excerpt || '')}
        </li>
      `).join('')}
    </ul>` : ''}
